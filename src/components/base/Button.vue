...
const commonClasses = 'whitespace-nowrap shadow inline-flex items-center justify-center border border-transparent rounded-md font-semibold focus:outline-none focus:shadow-outline transition duration-150 ease-in-out'
...
<template>
    <button type="button" @click="handleClick" :disabled="disabled" :class="`${props.disabled ? 'opacity-50 cursor-not-allowed' : ''} ${commonClasses} ${themeClasses[props.theme]} ${sizes[props.size]}`">
